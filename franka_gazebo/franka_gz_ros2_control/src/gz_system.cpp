...
struct jointData {
  ...
  gz_ros2_control::GZSystemInterface::ControlMethod joint_control_method;
};
...
class gz_ros2_control::GZSystemPrivate {
 public:
  GZSystemPrivate() = default;
...
bool GZSystem::initSim(const ModelKDL& kdl_model,
  ...
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    ...
    if (!_ecm.EntityHasComponentType(simjoint, gz::sim::components::JointPosition().TypeId())) {
      _ecm.CreateComponent(simjoint, gz::sim::components::JointPosition());
    }

    // Create joint velocity component if one doesn't exist
    if (!_ecm.EntityHasComponentType(simjoint, gz::sim::components::JointVelocity().TypeId())) {
    ...
    for (unsigned int i = 0; i < joint_info.state_interfaces.size(); ++i) {  // NOLINT
    ...
    for (unsigned int i = 0; i < joint_info.command_interfaces.size(); ++i) {  // NOLINT
...
void GZSystem::registerSensors(const hardware_interface::HardwareInfo& hardware_info) {
...
CallbackReturn GZSystem::on_init(const hardware_interface::HardwareInfo& system_info) {
  if (!is_sim_init) {
    RCLCPP_WARN(rclcpp::get_logger("GZSystem"),
                "%s: initSim() has not been called, "
                "dont care if another initSim() of GZSystem was called.",
                get_name().c_str());
  }
...
CallbackReturn GZSystem::on_deactivate(const rclcpp_lifecycle::State& previous_state) {
  return CallbackReturn::SUCCESS;
  return hardware_interface::SystemInterface::on_deactivate(previous_state);
}

hardware_interface::return_type GZSystem::read(const rclcpp::Time& /*time*/,
  ...
  for (unsigned int i = 0; i < this->dataPtr->joints_.size(); ++i) {
    ...
    const auto* jointVelocity = this->dataPtr->ecm->Component<gz::sim::components::JointVelocity>(
    ...
    const auto* jointPositions = this->dataPtr->ecm->Component<gz::sim::components::JointPosition>(
        this->dataPtr->joints_[i].sim_joint);
    ...
    this->dataPtr->joints_[i].joint_effort =
        force_or_torque.dot(gz::physics::Vector3d{this->dataPtr->joints_[i].joint_axis.Xyz()[0],
  ...
  for (unsigned int i = 0; i < this->dataPtr->imus_.size(); ++i) {
    if (this->dataPtr->imus_[i]->topicName.empty()) {
      auto sensorTopicComp = this->dataPtr->ecm->Component<gz::sim::components::SensorTopic>(
...
hardware_interface::return_type GZSystem::perform_command_mode_switch(
    const std::vector<std::string>& start_interfaces,
    const std::vector<std::string>& stop_interfaces) {
  if (!is_sim_init) {
    return hardware_interface::return_type::OK;
  }
  for (unsigned int j = 0; j < this->dataPtr->joints_.size(); j++) {  // NOLINT
...
hardware_interface::return_type GZSystem::write(const rclcpp::Time& /*time*/,
                                                const rclcpp::Duration& /*period*/) {
  if (!is_sim_init) {
    return hardware_interface::return_type::OK;
  }
      ...
      if (!this->dataPtr->ecm->Component<gz::sim::components::JointVelocityCmd>(
        ...
        const auto jointVelCmd =
            this->dataPtr->ecm->Component<gz::sim::components::JointVelocityCmd>(
                this->dataPtr->joints_[i].sim_joint);
        *jointVelCmd =
            gz::sim::components::JointVelocityCmd({this->dataPtr->joints_[i].joint_velocity_cmd});
      }
      ...
      if (!this->dataPtr->ecm->Component<gz::sim::components::JointForceCmd>(
        ...
        const auto jointEffortCmd =
            this->dataPtr->ecm->Component<gz::sim::components::JointForceCmd>(
                this->dataPtr->joints_[i].sim_joint);
        *jointEffortCmd =
            gz::sim::components::JointForceCmd({this->dataPtr->joints_[i].joint_effort_cmd});
    ...
    for (const auto& mimic_interface : mimic_joint.interfaces_to_mimic) {
      if (mimic_interface == "position") {
        ...
        if (vel == nullptr) {
          this->dataPtr->ecm->CreateComponent(
              this->dataPtr->joints_[mimic_joint.joint_index].sim_joint,
              gz::sim::components::JointVelocityCmd({velocity_sp}));
        } else if (!vel->Data().empty()) {
