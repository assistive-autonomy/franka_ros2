...
static constexpr size_t kBaseStateInterfaces = 4 * 2;  // base joints: pos + vel
static constexpr size_t kArmStateInterfaces = 7 * 2;   // arm joints: pos + vel
...
void MobileFr3DuoJointImpedanceExampleController::updateMobileBaseCommand(const rclcpp::Duration&) {
  ...
  RCLCPP_INFO_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(), 1000,
                       "Sending velocities: vx=%.3f, vy=%.3f, vz=%.3f, wx=%.3f, wy=%.3f, wz=%.3f",
                       v_x, v_y, 0.0, 0.0, 0.0, 0.0);
