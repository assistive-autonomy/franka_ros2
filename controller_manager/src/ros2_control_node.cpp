...
int main(int argc, char** argv) {
  ...
  RCLCPP_INFO(cm->get_logger(), "Spawning %s RT thread with scheduler priority: %d", cm->get_name(),
              thread_priority);

  std::thread cm_thread([cm, thread_priority, use_sim_time, enable_overrun]() {
    ...
    if (cm->get_parameter("cpu_affinity", cpu_affinity_param)) {
      ...
      if (cpu_affinity_param.get_type() == rclcpp::ParameterType::PARAMETER_INTEGER) {
        ...
        std::for_each(cpu_affinity_param_array.begin(), cpu_affinity_param_array.end(),
